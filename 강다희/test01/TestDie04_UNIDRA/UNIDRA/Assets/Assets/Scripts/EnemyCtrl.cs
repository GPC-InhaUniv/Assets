...
public class EnemyCtrl : MonoBehaviour
{
    ...
    private State state = State.Walking;        // 현재 스테이트.
    private State nextState = State.Walking;    // 다음 스테이트.
    ...
    private void Damage(AttackArea.AttackInfo attackInfo)
    {
        // 이펙트 발생.
    ...
    [RPC]
    private void DamageMine(int damage)
    {
        status.HP -= damage;
        if (status.HP <= 0)
        {
