...
{
    // 게임 타입 이름.
    private const string GameTypeName = "UNIDRA_die"; // 원하는 타이틀 이름으로 바꿔 쓰자.

    // 로컬 IP 주소와 포트.
    private const string LocalServerIP = "127.0.0.1"; // 개발용.
    ...
    private Status _status = Status.NoError;
    ...
    private bool useNat = false; // NAT 펀치스루를 사용하는가.
    ...
    // 서버를 실행하는 코루틴.
    private IEnumerator LaunchServerCoroutine(string roomName)
    {
    ...
    private void SetMasterServerAndNatFacilitatorIP(string masterServerAddress, string facilitatorAddress)
    {
    ...
    private void NetworkSettingWindow(int windowID)
    {
        // 플레이어 이름을 설정한다.
