...
namespace SlimeRace
{
    public partial class Form1 : Form
    {
        ...
        {
            slimes[0] = new Slime(89, 900, pictureBox1, "핑크슬라임");
            slimes[1] = new Slime(89, 900, pictureBox2, "태비슬라임");
            slimes[2] = new Slime(89, 900, pictureBox3, "헌터슬라임");
            slimes[3] = new Slime(89, 900, pictureBox4, "붐슬라임");

            guys[0] = new Guy("박준명", null, 75, radioButton1, label6);
            guys[1] = new Guy("이신재", null, 85, radioButton2, label7);
            guys[2] = new Guy("김판주", null, 65, radioButton3, label8);

            guys[0].UpdateLabels();     //레이블 초기 표시
            guys[1].UpdateLabels();    
            guys[2].UpdateLabels();
            pictureBox6.Hide();         //이벤트 이미지 숨김
            pictureBox7.Hide();
        }

        private void radioButton1_CheckedChanged(object sender, EventArgs e)   //라디오 첫번째 버튼 체크시  
        {
            label3.Text = guys[0].name;    //레이블 내용이 변경;
            numericUpDown1.Value = 5;      // 버튼이 달라질 시 베팅 값 최솟값으로 초기화
        }

        private void radioButton2_CheckedChanged(object sender, EventArgs e)   //라디오 두번째 버튼 체크시  
        {
            label3.Text = guys[1].name; ;
            numericUpDown1.Value = 5;     
        }

        private void radioButton3_CheckedChanged(object sender, EventArgs e)   //라디오 세번째 버튼 체크시  
        {
            label3.Text = guys[2].name; ;
            numericUpDown1.Value = 5;     
        }

        private void button1_Click(object sender, EventArgs e)                 //베팅 버튼
        {
            if (radioButton1.Checked)        //라디오 첫번째 버튼이 체크 되있을 경우
            {
                int amount = int.Parse(String.Concat(numericUpDown1.Value));    //베팅 금액을 받을 변수 선언.
                int slime = int.Parse(String.Concat(numericUpDown2.Value)) ;    //베팅한 슬라임 번호를 받을 변수 선언.
                guys[0].PlaceBet(amount, slime); //베팅의 내용으로 Bet 객체 생성.
                guys[0].UpdateLabels();          //그 내용을 레이블에 표시
            }
            else if (radioButton2.Checked)   //라디오 두번째 버튼이 체크 되있을 경우
            {
                int amount = int.Parse(String.Concat(numericUpDown1.Value));
                int slime = int.Parse(String.Concat(numericUpDown2.Value)) ;
        ...
        private void button2_Click(object sender, EventArgs e)   //레이스 버튼
        {
        ...
        private void pictureBox1_Click(object sender, EventArgs e)  //첫번째 슬라임 클릭시 정보
        {
            MessageBox.Show("이름: "+slimes[0].slimeName+ "\n\n특징: 그저 흔한 너무 귀여운 슬라임입니다.!", "1번 슬라임정보");
        }

        private void pictureBox2_Click(object sender, EventArgs e)  //두번째 슬라임 클릭시 정보
        {
            MessageBox.Show("이름: " + slimes[1].slimeName + "\n\n특징: 고양이 모습을 한 이 치명적 슬라임은 \n일반속도는 느리지만 가끔 높은 점프를 합니다.!", "2번 슬라임정보");
        }

        private void pictureBox3_Click(object sender, EventArgs e)  //세번째 슬라임 클릭시 정보
        {
            MessageBox.Show("이름: " + slimes[2].slimeName + "\n\n특징: 날쎈 모습만큼 빠른 속도를 가지고 있지만\n선천적으로 기복이 있는 편입니다.!", "3번 슬라임정보");
        }

        private void pictureBox4_Click(object sender, EventArgs e)  //네번째 슬라임 클릭시 정보
        {
        ...
        {
            pictureBox6.Hide();                   //기존에 있던 럭키이미지 숨기기 
            pictureBox7.Hide();                   //기존에 있던 타르이미지 숨기기 
