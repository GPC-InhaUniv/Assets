using UnityEngine;
using System.Collections;

public class PlayerController : MonoBehaviour
{
    public float moveStartDistance = 10f;
    public float moveForce = 150f;
    public float maxSpeed = 100f;
    float targetPointX;
    bool facingRight = true;
    new Rigidbody2D rigidbody2D;
    Animator myAnimator;
    public Camera maincamera;
    Vector3 plusXpos;
    Vector3 minusXpos;
    bool charaterHit = false;
    PlayerStatus playerStatus;
    void Start()
    {
        Vector3 screen_point = Camera.main.WorldToScreenPoint(transform.position);
        plusXpos = maincamera.transform.position;
        plusXpos.x+=0.06f;
        minusXpos = maincamera.transform.position;
        minusXpos.x -= 0.06f;
        targetPointX = screen_point.x;
        rigidbody2D = GetComponent<Rigidbody2D>();
        myAnimator= GetComponent<Animator>();
        playerStatus = GetComponent<PlayerStatus>();
       

    }

    void Update()
    {
        if (transform.position.y <= -100)
            Destroy(gameObject);

        if (charaterHit)
        {
            if (maincamera.transform.position.x == 0.0f)
                maincamera.transform.position = plusXpos;
            else if (maincamera.transform.position.x == plusXpos.x)
                maincamera.transform.position = minusXpos;
            else if (maincamera.transform.position.x == minusXpos.x)
            {
                maincamera.transform.position = new Vector3(0, 0,-10);
                charaterHit = false;
            }

        }

        if (!Input.GetMouseButtonDown(0))
            return;
        targetPointX = Input.mousePosition.x;

    }

    void FixedUpdate()
    {
        // 3D 좌표를 스크린 좌표로 변환한다.
        Vector3 screen_point = Camera.main.WorldToScreenPoint(transform.position);

        // 이동할 거리가 일정 거리 이하라면 이동 처리를 하지 않는다.
        if (Mathf.Abs(targetPointX - screen_point.x) <= moveStartDistance)
            return;

        // 이동할 곳이 왼쪽인지 오른쪽인지 계산해서 그 방향으로 이동하는 힘을 더한다.
        float horizontal = Mathf.Sign(targetPointX - screen_point.x);
        rigidbody2D.AddForce(Vector2.right * horizontal * moveForce);
        // 이동 속도를 제한한다.
        if (Mathf.Abs(rigidbody2D.velocity.x) > maxSpeed)
            rigidbody2D.velocity = new Vector2(Mathf.Sign(rigidbody2D.velocity.x) * maxSpeed, rigidbody2D.velocity.y);

        // 플레이어의 방향을 조정한다.
        if ((horizontal > 0 && !facingRight) || (horizontal < 0 && facingRight))
        {
            facingRight = !facingRight;
            Vector3 local_scale = transform.localScale;
            local_scale.x *= -1;
            transform.localScale = local_scale;
        }

    

    }
    
    void OnCollisionEnter2D(Collision2D collision)
    {
        if (collision.gameObject.tag == "Fire")
        {
            charaterHit = true;
            myAnimator.SetTrigger("Damage");
            playerStatus.Damage();
        }
    }

  
}