...
public class FollowCamera : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        ...
        float deltaY = inputManager.GetCameraDiretion().y;
        float deltaX = inputManager.GetCameraDiretion().x;
        horizontalAngle += deltaY * anglePerPixel;
        horizontalAngle = Mathf.Repeat(horizontalAngle, 360.0f);
        verticalAngle -= deltaX * anglePerPixel;
        verticalAngle = Mathf.Clamp(verticalAngle, -30.0f, 60.0f);
