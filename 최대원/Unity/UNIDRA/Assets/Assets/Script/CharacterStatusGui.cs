...
public class CharacterStatusGui : MonoBehaviour {
    ...
    //이름
    Rect nameRect = new Rect(0f, 0f, 12f, 24f);
    public GUIStyle nameLabelStyle;

    //체력 바
    public Texture backLifeBarTexture;
    public Texture frontLifrBarTexture;
    ...
    void DrawPlayerStatus()
    {
        ...
        foreach (PlayerCtrl item in players)
        {
            CharacterStatus status = item.GetComponent<CharacterStatus>();
            if(status != null)
            {
                DrawCharacterStatus(x, y, status, playerLifeBarRect, playerFrontLifeBarColor);
                y += delta_height;
            }
        }
    }
    ...
    void DrawEnemyStatus()
    {
        if(playerStatus.lastAttackTarget != null)
        {
            ...
            DrawCharacterStatus((baseWidth - enemyLifeBarRect.width) / 2.0f, 0f, target_status, enemyLifeBarRect, enemyFrontLifeBarColor);
        }
    }
    ...
    {
        GUI.Label(new Rect(x, y, nameRect.width, nameRect.height), status.characterName, nameLabelStyle);
        ...
        if (frontLifrBarTexture != null)
        {
            ...
            GUI.DrawTexture(new Rect(x + resize_front_bar_offset_x, y, front_bar_width * life_value, bar_rect.height), frontLifrBarTexture);
            GUI.color = gui_color;
        }
    }
