...
namespace Invaders
{
    ...
    class Game
    {
        ...
        private void ShotCtrl()
        {
            ...
            ReturnFire(invaderCols.ElementAt(select).First());
        ...
        private void CollideCheck()
        {
            ...
            // 플레이어 충돌 처리
            for (int i = invaderShots.Front; i != invaderShots.Rear; i++, i %= invaderShots.Size)
                if (playerShip.Area.Contains(invaderShots.GetData(i).Location)) LivesLeft--;
        ...
        private bool ReturnFire(Invader attacker)
        {
            if (invaderShots.IsFull()) return false;
            invaderShots.Push(attacker.Location);
            return true;
        }
        ...
        public Game(Rectangle boundaries, Random random)
        {
            ...
            invaderShots = new CircleQueue(Wave + 3);
